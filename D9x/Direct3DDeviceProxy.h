...
// ================= DETECTED GLYPH (للـ Learning) =================
struct DetectedGlyph {
    unsigned int hash;
    float x, y;
};
...
private:
    ...
    LONG m_refCount;
    IDirect3DBaseTexture9* m_texStages[3];

    bool IsLikelyTextTexture(IDirect3DBaseTexture9* tex);
    bool HasTextTexture();

    // ================= OCR FUNCTIONS =================
    unsigned int HashGlyph(BYTE* pixels, int pitch, int x, int y, int w, int h);
    void SaveGlyphToBMP(unsigned int hash, BYTE* pixels, int pitch, int x, int y, int w, int h);
    GlyphUV ProcessGlyph(IDirect3DTexture9* tex9, VertexUV* verts);
    wchar_t GetCharFromHash(unsigned int hash);
    void AddGlyph(unsigned int hash, float x, float y, int h);
    void FlushText();

    // ================= LEARNING FUNCTIONS =================
    float CompareStrings(const std::wstring& a, const std::wstring& b);
    void AttemptLearning(const std::vector<DetectedGlyph>& glyphs, const std::wstring& text);
    void SaveFontMap();
    void LoadFontMap();

public:
    HookedIDirect3DDevice9(IDirect3DDevice9* pReal);
    ~HookedIDirect3DDevice9();
    ...
    // Texture
    HRESULT STDMETHODCALLTYPE SetTexture(DWORD Sampler, IDirect3DBaseTexture9* pTexture);
    HRESULT STDMETHODCALLTYPE GetTexture(DWORD Sampler, IDirect3DBaseTexture9** ppTexture);

    // Draw Methods
    HRESULT STDMETHODCALLTYPE DrawPrimitive(D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex, UINT PrimitiveCount);
    HRESULT STDMETHODCALLTYPE DrawIndexedPrimitive(D3DPRIMITIVETYPE Type, INT BaseVertexIndex, UINT MinVertexIndex, UINT NumVertices, UINT StartIndex, UINT PrimitiveCount);
    ...
    // All IDirect3DDevice9 methods (pass-through)
    ...
    HRESULT STDMETHODCALLTYPE GetStreamSourceFreq(UINT StreamNumber, UINT* pDivider);
